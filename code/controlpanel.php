...
{	
	...
	function optionsmenu() 
	{
		...
		if (array_key_exists('save_buttons', $_POST) && $_POST['save_buttons'] == 'save') 
		{
			...
			if ( !$this->createButtonSprite( $buttons ) ) 
			{
	...
	function CreateButtonSprite( $buttons )
	{
		$buttonArray = $this->CreateButtons( $buttons );
		...
		$image_width = count( $buttonArray ) * 100;
		$mergedButtons = imagecreate( $image_width, 25 );
			
		$backcolor = imagecolorallocate ($mergedButtons, 100, 100, 100);
		ImageColorTransparent($mergedButtons, $backcolor);
		...
		{	
			...
			{
				imagecopymerge($mergedButtons, $button, $x, 0, 0, 0, $image_width, 25, 100);				
				$x += 100;
			}			
			...
			$filename = $url . "/images/Buttons/ButtonSprite.gif";
			
			imagegif( $mergedButtons, $filename );
		
			imagedestroy( $mergedButtons );
			foreach ($buttonArray as $button) 
			{
				imagedestroy( $button );
			}
	...
	function CreateButtons( $buttons )
	{
		...
		{
			$image = $this->CreateButton( $button );
			
			if ($image != null)
			{
				array_push($buttonArray, $image);
			}
		}
	...
	function CreateButton( $button_text )
	{
		$url = get_bloginfo('template_url');
		$wp = get_bloginfo('wpurl');
		$url = '../' . substr( $url, strlen ( $wp ) );
		...
		try {
			...
			$text_height = $textbox[5]-$text_box[1]; //upper right corner - lower right corner
